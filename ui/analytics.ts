...
/**
 * Everything we turn off, as data rather than as a line buried in a call, so a test can assert that
 * capture stays explicit. The host and the key are the only parts that vary at runtime.
 */
export const CAPTURE_POLICY = {
    ...
    // Replay is on, but it can never carry text. The editor paints real DOM spans holding the
    // customer's own copy (`canvas/render/backends.ts`), so an unmasked recording would be a video
    // of a confidential deck. Masked, it still shows layout, cursor, scroll and rage clicks, which
    // is the part worth having. Content control is here; the volume control is pauseReplay below.
