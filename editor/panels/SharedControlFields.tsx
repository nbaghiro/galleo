...
import { editorTokens } from "../core/store";
import { pickMedia, pickIcon } from "../core/media";
...
export const VectorField: Component<{ value?: Vector; onChange: (v: Vector) => void }> = (
    ...
    const nodes = (): number => props.value?.nodes.length ?? 0;
    const apply = (svg: string): void => {
        setText(svg);
        if (svg.trim()) props.onChange(parseSvg(svg));
    };
    return (
        <div class="flex flex-col gap-1.5">
            <TextArea value={text()} placeholder="Paste SVG markup…" onChange={apply} />
            <span class="text-[11px] text-soft">
                {nodes() > 0
                    ? `${nodes()} element${nodes() === 1 ? "" : "s"} imported`
                    : "No graphic yet"}
            </span>
        </div>
    );
...
export const Field: Component<{
    ...
    const control = (): JSX.Element => (
        ...
        >
            ...
            <Match when={f().control === "vector"}>
                <VectorField
                    value={props.value as Vector | undefined}
