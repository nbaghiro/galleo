...
const heading = "mb-2.5 mt-4 border-t border-line pt-3 text-[10px] font-semibold uppercase tracking-wider text-muted";
...
const Seg: Component<{ value: string; options: { label: string; value: string }[]; onChange: (v: string) => void }> = (
    props,
) => (
...
export const SectionInspector: Component<{ section: string }> = (props) => {
    ...
    const grad = (): { from: string; to: string; angle?: number } => bg().gradient ?? { from: "#000000", to: "#ffffff", angle: 135 };

    const setGrid = (id: string): void => commit(setSectionGrid(editor.artifact, props.section, id));
    const setBleed = (v: boolean): void => commit(setSectionBleed(editor.artifact, props.section, v));
    ...
    const setKind = (kind: string): void => {
        const t = resolveTheme(editor.artifact.theme).tokens;
        if (kind === "color") setBg({ kind, color: bg().color ?? t.accent });
        else if (kind === "gradient") setBg({ kind, gradient: bg().gradient ?? { from: t.accent, to: t.surface, angle: 135 } });
        else if (kind === "image") setBg({ kind, image: bg().image ?? "https://picsum.photos/seed/galleo-bg/1600/1000", scrim: bg().scrim ?? 0.45 });
        else setBg({ kind: "none" });
    };

    return (
        <div>
            <div class="font-mono text-[11px] font-semibold uppercase tracking-wider text-muted">Section</div>
            ...
            <div class="mt-3 flex flex-col gap-2">
                <Switch>
                    <Match when={bg().kind === "color"}>
                        <input type="color" class="h-8 w-full cursor-pointer rounded border border-line bg-canvas" value={bg().color ?? "#000000"} onInput={(e) => setBg({ color: e.currentTarget.value })} />
                    </Match>
                    <Match when={bg().kind === "gradient"}>
                        <div class="flex gap-2">
                            <input type="color" class="h-8 flex-1 cursor-pointer rounded border border-line" value={grad().from} onInput={(e) => setBg({ gradient: { ...grad(), from: e.currentTarget.value } })} />
                            <input type="color" class="h-8 flex-1 cursor-pointer rounded border border-line" value={grad().to} onInput={(e) => setBg({ gradient: { ...grad(), to: e.currentTarget.value } })} />
                        </div>
                        <label class="text-[11px] text-muted">Angle</label>
                        <input type="range" min={0} max={360} step={5} class="accent-accent" value={grad().angle ?? 135} onInput={(e) => setBg({ gradient: { ...grad(), angle: Number(e.currentTarget.value) } })} />
                    </Match>
                    <Match when={bg().kind === "image"}>
                        <input class={inputCls} placeholder="https://… image url" value={bg().image ?? ""} onInput={(e) => setBg({ image: e.currentTarget.value })} />
                        <label class="text-[11px] text-muted">Darken (scrim)</label>
                        <input type="range" min={0} max={0.8} step={0.05} class="accent-accent" value={bg().scrim ?? 0.45} onInput={(e) => setBg({ scrim: Number(e.currentTarget.value) })} />
                    </Match>
                </Switch>
            </div>
        </div>
