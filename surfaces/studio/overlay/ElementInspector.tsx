...
export const ElementInspector: Component<{ address: ElementAddress }> = (props) => {
    ...
    return (
        <div>
            ...
            <div class="mb-4">
                <div class="mb-2.5 text-[10px] font-semibold uppercase tracking-wider text-muted">
                    Layout
                </div>
                ...
                <Show when={widthMode() === "pct"}>
                    <Field
                        field={PCT_FIELD}
                        value={pct()}
                        onChange={(v) => setLayout({ width: { pct: Number(v) } })}
                    />
                </Show>
                <Field
                    field={HEIGHT_FIELD}
                    value={layout().height ?? "fit"}
                    onChange={(v) => setLayout({ height: v as "fit" | "fill" })}
                />
                <Field
                    field={ALIGN_FIELD}
                    value={layout().align ?? "start"}
                    onChange={(v) => setLayout({ align: v as "start" | "center" | "end" })}
                />
            </div>
            <Show
