...
export function generateFromPrompt(prompt: string, themeId: string): ArtifactContent {
    ...
    return deck(themeId, [
            ...
            "full",
            {
                a: cell(
                    group(
                        t("GENERATED", "eyebrow"),
                        t(cap, "display"),
                        t("A starting point — edit anything, switch the theme, or present it.", "lead"),
                    ),
                ),
            },
            { bleed: true, background: bgImage(`${seed}-cover`, 0.5) },
        ),
        section("g2", "split-6040", {
            a: cell(
                group(
                    ...
                    t("The big idea", "h2"),
                    t(`Replace this with the core of ${title}. Galleo drafted the structure; you bring the substance.`, "body"),
                ),
            ),
        ...
        section("g4", "full", { a: cell(quote("Generated in seconds. Made yours in minutes.", `— ${title}`)) }),
        section("g5", "split-4060", {
            ...
            b: cell(
                group(
                    ...
                    t("Three steps", "h2"),
                    bullets("Describe it — Galleo drafts the outline", "Drag in elements, edit text inline", "Theme it and present"),
                ),
            ),
        }),
        section("g6", "split-4060", {
            a: empty,
            b: cell(group(t("Next", "eyebrow"), t("Make it yours.", "h2"), button("Start editing"))),
        }),
    ]);
}
