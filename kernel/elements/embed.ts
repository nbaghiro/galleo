...
export const embedElement: ElementSpec<EmbedData> = {
    ...
    layout: (d: EmbedData, ctx: LayoutCtx): EngineNode => ({
        ...
        children: [
            { w: fit(), h: fit(), text: { text: "🔗", fontId: fontStack("ui", ctx.theme), size: 20, color: ctx.theme.muted, align: "center", wrap: "none" } },
            {
                w: grow(),
                h: fit(),
                direction: "col",
                gap: 3,
                children: [
                    { w: grow(), h: fit(), text: { text: d.title ?? "Embedded link", fontId: fontStack("ui", ctx.theme), size: 15, weight: 600, color: ctx.theme.ink, align: "start", wrap: "words" } },
                    { w: grow(), h: fit(), text: { text: d.url ?? "https://…", fontId: fontStack("mono", ctx.theme), size: 12, color: ctx.theme.muted, align: "start", wrap: "none" } },
                ],
            },
        ],
    }),
