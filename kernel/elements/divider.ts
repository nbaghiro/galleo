...
export const dividerElement: ElementSpec<DividerData> = {
    ...
    layout: (d: DividerData, ctx: LayoutCtx): EngineNode => {
        ...
        return { w: grow(), h: fixed(th), fill: { color: d.color ?? ctx.theme.line, radius: Math.max(0.5, th / 2) } };
    },
    controls: [
        { key: "thickness", label: "Thickness", control: "slider", min: 1, max: 10, step: 1, unit: "px" },
