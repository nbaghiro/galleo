...
> rendering core. Companion to `design/product-direction.md`. Supersedes the CSS-arrangement
> approach used in the `design/editor-explorer.html` prototype.
...
- **Text editing:** contenteditable over engine-positioned boxes, or our own caret model? (Affects
  how the `MeasureText` loop integrates with live typing.)
- **Multi-column magazine balancing** — how good does v1 need to be?
- **PPTX** — is "approximate text" acceptable, or do we commit to fixed-box/outline export?
- **Where layout runs for export** — client (reuse the TS engine) vs a server worker (WASM)?
