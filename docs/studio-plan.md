...
**The canvas is layout-agnostic.** It is _not_ a deck of paginated slides with a deck/doc/web
...
- **Right panel** — context-dependent:
    - nothing selected → **element palette** (the block/element library you _drag from_),
...
- **cell edge** → _split_ the cell into a new column/row (grows the grid),
...
paginates to pages. (A section may _optionally_ opt into a fixed aspect ratio later if a user wants
