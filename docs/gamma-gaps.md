...
This is the #1 reason we don't _look_ like Gamma yet.
...
| **Section backgrounds** — image, color, gradient behind the whole section, with text overlaid | Section bg = theme `surface` only; images live _inside_ a cell | 🟥 per-section background (image/color/gradient) + content overlay + scrim |
...
Gamma edits mostly _on the canvas_; we lean on the right-panel inspector.
...
## Recommended order to close the _visual_ gap fastest
