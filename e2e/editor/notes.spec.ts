...
// The strip these drive does not exist: nothing renders a "Speaker notes and narration"
// control, so the script can only be written by the AI today. Marked fixme rather than left
// out of the repo, so the intended behaviour is recorded and runs the moment it is built.
test.fixme("the notes strip opens from the section rail and edits save", async ({ page }) => {
...
test.fixme("the strip closes again, and the toggle is not in the header bar", async ({ page }) => {
