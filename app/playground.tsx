...
import { previewSvg } from "@studio/editing/element-previews";
import { measureText } from "@studio/canvas/measure";
import { ctxFor, layoutNode } from "@studio/canvas/render";
...
const txt = (text: string, style: string): ElementInstance => ({
    type: "text",
    data: { text, style },
});
...
const FILLED: Record<string, unknown> = {
    ...
    image: {
        src: "https://picsum.photos/seed/galleo-library/1100/760",
        aspect: 1.45,
        radius: 12,
        fit: "cover",
    },
    ...
    group: {
        children: [
            txt("THE FEELING", "eyebrow"),
            txt("Your taste, on autopilot", "h2"),
            txt(
                "No more scrolling for the right track — it reads the room and plays the thing.",
                "body",
            ),
        ],
    },
    ...
    bullets: {
        children: [
            txt("Curated, never generated slop", "body"),
            txt("Edit any block inline", "body"),
            txt("Switch theme in one click", "body"),
            txt("Present or publish instantly", "body"),
        ],
        marker: "check",
    },
    ...
    quote: {
        children: [
            txt("Speed made everyone a publisher. Taste is the only moat left.", "title"),
            txt("— the Galleo thesis", "byline"),
        ],
    },
    callout: {
        tone: "success",
        children: [txt("Shipped: document backgrounds now render behind every section.", "body")],
    },
    code: {
        code: 'const artifact = await galleo.generate(prompt);\nrender(artifact, { theme: "noir" });',
    },
    badge: { text: "BETA" },
    spacer: { height: 28 },
...
function previewFrame(node: EngineNode, width: number, tk: Tokens, label: string): HTMLElement {
    ...
    const box = el(
        "div",
        `background:${tk.surface};border:1px solid ${tk.line};border-radius:14px;padding:18px;width:max-content`,
    );
    box.appendChild(inner);
    const cap = el(
        "div",
        `font:600 9.5px/1.4 ui-monospace,monospace;letter-spacing:.09em;text-transform:uppercase;color:${tk.muted};margin-bottom:9px`,
        label,
    );
...
function svgFrame(type: string, tk: Tokens, label: string): HTMLElement {
    ...
    const box = el(
        "div",
        `background:${tk.surface};border:1px solid ${tk.line};border-radius:14px;padding:12px`,
    );
    box.appendChild(inner);
    const cap = el(
        "div",
...
function elementCard(type: string, width: number, tk: Tokens): HTMLElement {
    ...
    const card = el(
        "div",
        `border:1px solid ${tk.line};border-radius:18px;padding:22px;background:${tk.bg}`,
    );
    ...
    head.append(
        ...
        el(
            "div",
            `font:500 11px/1 ui-monospace,monospace;color:${tk.muted}`,
            `${spec.type} · ${spec.category} · ${spec.tier}`,
        ),
    );
    ...
    states.append(
        ...
        previewFrame(
            composeInstance({ type, data: FILLED[type] ?? spec.create() }, ctx),
            width,
            tk,
            "Filled",
        ),
...
function render(): void {
    ...
    const bar = el(
        "div",
        `position:sticky;top:0;z-index:10;display:flex;align-items:center;gap:18px;flex-wrap:wrap;padding:16px 28px;background:${tk.surface};border-bottom:1px solid ${tk.line}`,
    );
    bar.append(
        el(
            "div",
            `font:800 16px/1 system-ui,sans-serif;color:${tk.ink}`,
            "Galleo · Element Library",
        ),
    );
    bar.append(
        el(
            "div",
            `font:500 12px/1 ui-monospace,monospace;color:${tk.muted}`,
            `${listElements().length} elements`,
        ),
    );

    const themeSel = el(
        "select",
        `margin-left:auto;padding:7px 10px;border-radius:9px;border:1px solid ${tk.line};background:${tk.bg};color:${tk.ink};font:500 13px system-ui`,
    ...
    const widthGroup = el(
        "div",
        `display:flex;gap:2px;padding:3px;border-radius:10px;border:1px solid ${tk.line};background:${tk.bg}`,
    );
    for (const [name, w] of Object.entries(WIDTHS)) {
        const active = w === width;
        const b = el(
            "button",
            `padding:6px 12px;border:0;border-radius:7px;cursor:pointer;font:600 12px system-ui;background:${active ? tk.accent : "transparent"};color:${active ? tk.onAccent : tk.muted}`,
            name,
        );
        b.onclick = () => {
            width = w;
    ...
    const cats = [
        ...CATEGORY_ORDER,
        ...items.map((s) => s.category).filter((c) => !CATEGORY_ORDER.includes(c)),
    ];
    ...
    for (const cat of cats) {
        ...
        main.append(
            el(
                "div",
                `margin:26px 0 14px;font:700 11px/1 ui-monospace,monospace;letter-spacing:.12em;text-transform:uppercase;color:${tk.muted}`,
