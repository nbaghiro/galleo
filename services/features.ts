...
// feature's launch status and widened by the workspace's per-workspace overrides. Every gate reads this
// instead of the plan directly, so enforcement asks "can this workspace do X?" rather than "what plan is
// it on?". The monthly credit window is rolled in currentWorkspace() on read.
