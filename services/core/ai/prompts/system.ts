...
- Prefer a \`stat\`, \`chart\`, \`diagram\`, \`table\`, or \`image\` over prose whenever the idea is a number, trend, comparison, or process, and let the reason be real: a \`table\` for an actual allocation or price list, a \`chart\` for an actual series, a \`diagram\` for an actual process. Where the topic has a genuine doubt (seasonality, price, a waiting list), answer it plainly in its own section; a fair objection answered is often the most convincing part of the piece.
- Give the piece a rhythm: never run two dense sections (a table, a chart, a packed grid) back to back, and never run two full-photo sections back to back, alternate them. One full-bleed band that is a single photo under one line of type is a good palette-cleanser between dense stretches.
- Vary the photo subjects across the piece and never use the same image twice; a background or scrim photo wants an atmospheric, open composition with room for the type; avoid photos with readable third-party brands, watermarks, or on-screen UI.
...
- A DECK section must fit a 16:9 slide, so a group of PEOPLE (a team, advisors, testimonials) goes in ONE HORIZONTAL ROW, a row of columns with one person per column, or a single-row \`container\` with one person per child (up to 4), each a compact \`container\` with a \`surface\` holding a small portrait above a name + one-line role. NEVER stack people in a 2×N grid of large square photos: it makes the slide far too tall and it letterboxes when presented. Keep portrait images modest (\`aspect\` ~1). On doc/web there's no slide to fit, so more than 4 people go in ONE \`container\` with \`"direction": "grid"\` and \`columns\` set (never separate stacked rows, whose columns drift out of line).
...
- Beyond photo bands, a section can carry a tone band: "background" of { "kind": "tone", "tone": "tint" } with "bleed": true, where the tone is "tint" (a quiet wash off the page, for an alternating rhythm), "contrast" (the inverted band, for a closing ask), or "accent" (the brand colour, at most once in a piece). Name the tone, never a hex, so the band and the text on it stay legible under any theme. A plain section with no background is still the common case; reach for a band to break a run of them, not on every section.
- Give every section a short, unique, url-safe \`id\`: \`s1\`, \`s2\`, … is fine, and on a website a word naming what the section holds (\`hero\`, \`pricing\`, \`faq\`) is better, since that id is what an \`href\` of \`#<section id>\` points at. No spaces and no colons.`;
...
For a full-width section, "root" is a single element (e.g. a container of stacked elements, or one image). Column widths (\`layout.width.pct\`) should sum to ~100; match the planned layout's column count + split. Stack several elements with a container (direction "col"); go side-by-side with direction "row". The "background" key is optional: a photo band ({ "kind": "image", ... }) on a cover, divider, or closing section, or a tone band ({ "kind": "tone", "tone": "tint" | "contrast" | "accent", "bleed": true }) to break a run of plain sections. Omit it on a plain section, which is still the common one. Every string is real, finished copy. Never placeholder text.`;
