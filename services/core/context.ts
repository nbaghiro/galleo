...
export interface ContextItemMeta {
    ...
    chars: number;
    chunks: number;
    original: boolean; // a stored source file the inspector can render as-is
    createdAt: string;
}
...
export async function getContextItems(
    ...
    const rows = await db
        .select({
            ...
            chunks: sql<number>`count(${schema.chunks.id})::int`,
            original: sql<boolean>`(${schema.contextItems.original} is not null)`,
...
/** The original bytes of a server-extracted upload, kept so the inspector can show the real file. */
...
async function insertItem(
    ...
    const [row] = await db
        ...
        .values({
            ...
            body,
            chars: body.length,
            original: item.original?.data ?? null,
            originalMime: item.original?.mime ?? null,
            addedBy: userId,
        })
...
export async function addTextItem(
    ...
    await ownedContext(workspaceId, contextId);
    return insertItem(workspaceId, contextId, userId, { kind, title, body, original }, embed);
}

/** The stored original bytes of a server-extracted upload, for the inspector's real-file view. */
export async function getItemOriginal(
    ...
    const [row] = await db
        .select({
            original: schema.contextItems.original,
            mime: schema.contextItems.originalMime,
        })
        .from(schema.contextItems)
        .where(
    ...
    return row.original && row.mime ? { data: row.original, mime: row.mime } : null;
}
