...
| render backends (DOM+raster)   | `canvas/render/__tests__/{backends,backends.dom,window}`                                                             | real `document` (happy-dom), real inline styles, paint windowing (geometry parity, layer release, placeholder heights)                                              | `installCanvas2D`, image IO               | DONE     |
...
| route policies (integration)   | `services/api/__tests__/{session,artifacts,links,ai,context,folders,search,paging}.itest.ts`                         | real Postgres, real SQL, auth cookie, gating/shaping; search runs the real generated tsvector; paging covers the keyset walk, windowed reads, and section-op writes | DB (`galleo_test`), LLM/Stripe/mail/clock | DONE (8) |
...
| services features/models       | `services/__tests__/features.test.ts`, `services/ai/__tests__/models.test.ts`                                        | seat-scaled credit pools, tier→model mapping                                                                                                                        | —                                         | DONE     |
