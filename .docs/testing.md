...
| billing routes (integration)   | `services/api/__tests__/{billing,attribution}.itest.ts`, `services/core/__tests__/credits.itest.ts`               | real Postgres, credit engine (bonus-aware refunds, locked window roll, pool-then-bonus), server-priced spend, checkout guard + superseded-sub cancel, scheduled downgrades, webhooks, trials, seat floor incl. held invites, attribution + net mySpend + keyset paging | DB, Stripe SDK (`vi.mock("stripe")`)      | DONE     |
