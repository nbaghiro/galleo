...
| billing routes (integration)      | `services/api/__tests__/{billing,attribution}.itest.ts`, `services/core/__tests__/ledger.itest.ts`                                                                    | real Postgres, credit engine (single-pool charge/refund, the keyed roll on read for every plan, per-seat grants), server-priced spend and the free doorway gate, checkout guard, the four webhook cases incl. the grant on any subscription increase, seat floor incl. held invites, attribution + keyset paging                                                                                                                   | DB, Stripe SDK (`vi.mock("stripe")`)      | DONE     |
