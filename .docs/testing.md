...
| ------------------------------ | ----------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------- |
| model pure contract            | `model/__tests__/{theme,text,ai,credits,features,billing,tools,target,authoring,geometry,section,digest,content}` | color/text/pricing/protocol math, real `@themes`                                                                                                                                                                                                                                   | — (theme: `registerThemes`)               | DONE     |
...
| billing routes (integration)   | `services/api/__tests__/{billing,attribution}.itest.ts`, `services/core/__tests__/credits.itest.ts`               | real Postgres, credit engine (single-pool charge/refund, locked window roll, add-on-widened limits), server-priced spend, checkout guard + superseded-sub cancel, scheduled downgrades, webhooks, trials, seat floor incl. held invites, attribution + net mySpend + keyset paging | DB, Stripe SDK (`vi.mock("stripe")`)      | DONE     |
