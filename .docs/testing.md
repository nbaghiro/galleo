...
| --------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------- |
| model pure contract               | `model/__tests__/{artifact,artifact-access,theme,text,ai,credits,billing,eval,collab,analytics,speech,authoring,geometry,workspace,comments,element-ids,media-refs}` | color/text/pricing/protocol math, comment anchors + threads, the element-id walker, real `@themes`                                                                                                                                                                                                                                                                                                                                 | — (theme: `registerThemes`)               | DONE     |
...
| route policies (integration)      | `services/api/__tests__/*.itest.ts` (incl. `comments.itest.ts`)                                                                                                      | real Postgres, real SQL, auth cookie, gating/shaping; search runs the real generated tsvector; paging covers the keyset walk, windowed reads, section-op writes, and the element-id stamp; comments covers anchors, the thread rules, and the delete matrix                                                                                                                                                                        | DB (`galleo_test`), LLM/Stripe/mail/clock | DONE (8) |
...
| comment anchors (integration)     | `services/api/__tests__/comment-anchors.itest.ts`                                                                                                                    | a row written before element ids is stamped on read, once, under a lock; stamping is not an edit (no updatedAt, no seq); the digest and search text re-derive with it; a comment written against a served id is still resolvable on the next read, for both anchor kinds                                                                                                                                                           | DB (`galleo_test`)                        | DONE     |
...
- **`services/core/ai/run.ts` `writeSectionFrom`** — the auto-repair loop (bad-JSON → retry, `checkSection` trips
  → retry, throw after 2 fails). Composes real `extractJson` + `zSection` + `checkSection`.
- **`services/core/ai/run.ts` `runGenerate`** — assert the exact `TurnEvent` sequence (intake → outline → plan →
