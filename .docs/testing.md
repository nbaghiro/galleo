...
| ----------------- | ------------------------------------ | --------------- | ------------------------------------ | ---------------------------- | -------------------------- |
| **`unit`**        | `vitest.config.ts`                   | `**/*.test.ts`  | `node` (+ per-file happy-dom opt-in) | none (pure + injected seams) | the bulk of the suite      |
| **`integration`** | `vitest.integration.config.ts`       | `**/*.itest.ts` | `node`, `fileParallelism:false`      | real `galleo_test` Postgres  | the route + query coverage |
...
`// @vitest-environment happy-dom` docblock. Coverage instrumentation targets **every layer**
(`coverage.include` = model · canvas · ui · editor · app · services) so an untested area reads as 0%
instead of being invisible; it previously covered `canvas`+`model` only, which hid the rest. Excluded, with
reasons in the config: the raster/IO shells (§7), the offline eval harness, seed/template content, and
drizzle's generated migration metadata. Today: the layout engine ~**99%**, elements ~**90%**, model ~**95%**,
ui ~**86%**, editor core ~**55%**, services ~**15%** in this suite (its routes are covered by `integration`,
which reports separately), ~**50%** overall. `thresholds` are floors set just under those numbers, global
plus per-directory for `model/**`, `canvas/engine/**`, and `canvas/elements/**`, so a regression fails rather
than merely showing up in a report. `integration` is a separate config (no shared `projects` file yet)
exercising Hono routes against a throwaway Postgres, and runs as its own CI job with a Postgres service.
...
> LLM/Stripe/mail/clock (the real Postgres, real SQL, real auth cookie are not mocks). The embedding
> model joins that list through the `Embedder` seam: `context.itest.ts` runs real pgvector SQL with a
> deterministic keyword-axis embedder, so ranking is asserted without a paid API call.
...
- **Builders** — `sectionOf`, `artifactOf`, `textInst`, `imageInst`, `groupRow`/`groupCol` (thin wrappers
  over the real `@model/artifact` builders).
...
| --------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------- | -------- |
| model pure contract               | `model/__tests__/{artifact,artifact-access,theme,text,ai,credits,billing,trace,collab,analytics,speech,authoring,geometry,workspace,comments,element-ids,media-refs}` | color/text/pricing/protocol math, comment anchors + threads, the element-id walker, real `@themes`                                                                                                                                                                                                                                                                                                                                 | — (theme: `registerThemes`)               | DONE     |
| engine solver                     | `canvas/engine/__tests__/{layout,fragment,profile}`                                                                                                                   | Clay 3-pass solver, pagination, profile math                                                                                                                                                                                                                                                                                                                                                                                       | `measure`                                 | DONE     |
| elements ops + compose            | `canvas/elements/__tests__/{ops,compose,layouts,spec,blueprint,previews,coerce,reflow,outline,bullets,tokenscale,element-ids}`                                        | real registry, `DEFAULT_THEME.tokens`, profiles                                                                                                                                                                                                                                                                                                                                                                                    | `measure`                                 | DONE     |
...
| render backends (DOM+raster)      | `canvas/render/__tests__/{backends,backends.dom,window,placeholder}`                                                                                                  | real `document` (happy-dom), real inline styles, paint windowing, request policy, kind-aware stand-ins                                                                                                                                                                                                                                                                                                                             | `installCanvas2D`, image IO               | DONE     |
...
| editor dnd + inspect              | `editor/core/__tests__/dnd`, `editor/core/__tests__/infographic`                                                                                                      | `@elements` ops, real `ArtifactContent`                                                                                                                                                                                                                                                                                                                                                                                            | —                                         | DONE     |
| editor store + commands           | `editor/core/__tests__/{store,commands,clipboard}`, `editor/core/__tests__/{windowed,comment-layout}`                                                                 | history/selection/commit reducers, `applyPatch`, windowed placeholders + resolution + undo across a load, comment marker/chip placement + the run-layout offset math                                                                                                                                                                                                                                                               | clock (coalesce), injected transports     | DONE     |
| editor AI flows                   | `editor/core/__tests__/ai`                                                                                                                                            | ops, `applyPatch`, ranking                                                                                                                                                                                                                                                                                                                                                                                                         | injected transport                        | DONE     |
| app HTTP client                   | `app/__tests__/api`                                                                                                                                                   | URL/body build (incl. the `/me` account methods), response mapping, SSE parse                                                                                                                                                                                                                                                                                                                                                      | `fetch`                                   | DONE     |
| app route + stores                | `app/__tests__/route-context`, `app/stores/__tests__/{library,library-paging,search,save,comments}`                                                                   | route derivers, formatters, search cache + ranking, keyset page append/dedupe, per-card fetch, autosave section-op diffs + fallback, comment load/mutate/refetch + the flush before a create                                                                                                                                                                                                                                       | clock, `fetch`                            | DONE     |
| services auth                     | `services/utils/__tests__/auth`                                                                                                                                       | scrypt crypto round-trip, HMAC session sign/verify                                                                                                                                                                                                                                                                                                                                                                                 | env (`SESSION_SECRET`)                    | DONE     |
| services AI prompts               | `services/core/ai/prompts/__tests__/*` (14 files)                                                                                                                     | pure string assembly, real `ELEMENTS` catalog                                                                                                                                                                                                                                                                                                                                                                                      | —                                         | DONE     |
| services AI schema + quality      | `services/core/ai/__tests__/{schema,quality,locate,trace,meter}`                                                                                                      | real `zod`, deterministic gate                                                                                                                                                                                                                                                                                                                                                                                                     | —                                         | DONE     |
| services AI tools                 | `services/core/ai/tools/__tests__/{structure,manage,library,register,contract,relayout}`                                                                              | deterministic patch producers                                                                                                                                                                                                                                                                                                                                                                                                      | —                                         | DONE     |
| services media/mail/billing       | `services/core/__tests__/{models,context,extract,spend,pricing,templates,section-ops}`, `services/core/__tests__/mail`, `services/core/__tests__/stripe`              | pure dims/dispatch/escape/price mapping                                                                                                                                                                                                                                                                                                                                                                                            | env (price ids, stock keys)               | DONE     |
| ui primitives (pure)              | `ui/__tests__/{focus,fuzzy,keys,palette,gesture,viewport,analytics,motion}`                                                                                           | focus-trap, fuzzy match, keymap, palette-model (sections, sources, snippet runs)                                                                                                                                                                                                                                                                                                                                                   | —                                         | DONE     |
| route policies (integration)      | `services/api/__tests__/*.itest.ts` (incl. `comments.itest.ts`)                                                                                                       | real Postgres, real SQL, auth cookie, gating/shaping; search runs the real generated tsvector; paging covers the keyset walk, windowed reads, and section-op writes; comments covers anchors, the thread rules, and the delete matrix                                                                                                                                                                                              | DB (`galleo_test`), LLM/Stripe/mail/clock | DONE (8) |
| visual eval (deterministic)       | `scripts/__tests__/visual-eval.test.ts`, `pnpm eval:shots`                                                                                                            | archetype classification and rhythm over the hand-built corpus, role expectations, and the geometric checks (contrast, left edges, type scale). `eval:shots` renders the corpus in real Chromium so the height-derived checks (`fits-frame`, `fills-frame`) can be measured at all: ~70 webfonts drive `measureText`, `measureText` drives the solver, so a Node stand-in measures a layout nobody sees                            | real Chromium via Playwright              | DONE     |
| billing routes (integration)      | `services/api/__tests__/{billing,attribution}.itest.ts`, `services/core/__tests__/ledger.itest.ts`                                                                    | real Postgres, credit engine (single-pool charge/refund, locked window roll, add-on-widened limits), server-priced spend, checkout guard + superseded-sub cancel, scheduled downgrades, webhooks, trials, seat floor incl. held invites, attribution + net mySpend + keyset paging                                                                                                                                                 | DB, Stripe SDK (`vi.mock("stripe")`)      | DONE     |
| workspace roles (integration)     | `services/api/__tests__/roles.itest.ts`                                                                                                                               | role matrix per mutation, legacy editor mapping, invite-with-role, rename/leave/transfer edges                                                                                                                                                                                                                                                                                                                                     | DB (`galleo_test`)                        | DONE     |
| workspace routes (integration)    | `services/api/__tests__/workspace.itest.ts`                                                                                                                           | members/invites/accept/switch/remove + seat caps                                                                                                                                                                                                                                                                                                                                                                                   | DB                                        | DONE     |
| artifact access (integration)     | `services/api/__tests__/access.itest.ts`                                                                                                                              | real Postgres and real rows: the level matrix per route, `none` answering 404 rather than 403, the admin and creator floors, inherit-then-override, the library + search filters, comments per level, admin-only trash emptying, the publish policy, workspace-settings validation, and the per-member spend cap through the real `reserve`                                                                                        | DB (`galleo_test`)                        | DONE     |
| comment anchors (integration)     | `services/api/__tests__/comment-anchors.itest.ts`                                                                                                                     | the ids a read serves are the row's own and stable across reads; a comment written against a served id is still resolvable on the next read, for both anchor kinds, and across a section-op write                                                                                                                                                                                                                                  | DB (`galleo_test`)                        | DONE     |
| collaboration protocol            | `model/__tests__/collab.test.ts`                                                                                                                                      | frame guards on both ends, cursor encode/decode round-trip through mismatched boxes, lease keys, peer colours, and the assertion that no pixel reaches the wire                                                                                                                                                                                                                                                                    | —                                         | DONE     |
| collaboration room                | `services/__tests__/collab-room.test.ts`                                                                                                                              | the room with an injected applier and clock: join/leave/evict, lease claim/deny/release/TTL, ring-buffer catch-up vs resync, per-role rejection of ops and claims, two clients converging through the real `applySectionOps` with a colour-vs-text race, a write that throws rather than failing (the queue behind it still runs), and a level changed under an open socket                                                        | —                                         | DONE     |
| collaboration client              | `editor/core/__tests__/{collab-cursors,collab-sync}.test.ts`, `app/stores/__tests__/{collab,save-driver}.test.ts`                                                     | cursor encode/decode against fake engine output, roster and lease reductions, per-key unacked-local-wins, identity-preserving remote apply, the undo skip rule, deletion-wins session end, text-session checkpoints and the emit baseline, the viewer gate, pending keys cleared on a drop, socket lifecycle with backoff and heartbeat, the two-driver handover in both directions, the save retry policy and the conflict reload | fake socket, fake `fetch`                 | DONE     |
| collaborator grants (integration) | `services/api/__tests__/{collab-access,collab-socket}.itest.ts`                                                                                                       | grants end to end: the level matrix for an outsider, a grant narrowing and widening a member, the cross-workspace read, token invite and accept, revoke, cascade on delete, what a grant does not open (AI, trash, publish), the three list surfaces agreeing, an open room following a revoke, a lock, and a removal, and the socket upgrade's auth                                                                               | DB (`galleo_test`)                        | DONE     |
| account routes (integration)      | `services/api/__tests__/account.itest.ts`                                                                                                                             | real Postgres + real scrypt: the `/me` projection, profile rename, password change and first-set incl. the `password_changed_at` stamp and the reissued cookie, connections list/unlink with the last-credential refusal, prefs merge + normalization, memberships with roles, leave-by-id                                                                                                                                         | DB (`galleo_test`)                        | DONE     |
| oauth link path (integration)     | `services/api/__tests__/oauth.itest.ts`                                                                                                                               | real state/intent/session logic and real rows: intent-cookie gating, a link bound to the session rather than to the provider's email, the already-linked-elsewhere refusal, idempotent relink, the expired-session fallback to sign-in, per-intent error destinations                                                                                                                                                              | DB, `arctic` (`vi.mock`)                  | DONE     |
| media routes (integration)        | `services/api/__tests__/media.itest.ts`                                                                                                                               | storage cap on uploads (stored bytes only)                                                                                                                                                                                                                                                                                                                                                                                         | DB                                        | DONE     |
| services features/models          | `services/core/__tests__/models.test.ts`, `services/core/__tests__/models.test.ts`                                                                                    | seat-scaled credit pools, tier→model mapping                                                                                                                                                                                                                                                                                                                                                                                       | —                                         | DONE     |
...
_(The former `canvas/render/present.ts` and `canvas/render/export.ts` are gone — their arithmetic folded into
...
- **IO shells** — `download()`, `window.print()`, `canvas.toBlob`, `URL.createObjectURL`, `Image` decode,
  ...
  `canvas/render/export.ts`, tested) and leave only the shell, which is added to `coverage.exclude` with a
...
- **`write-section`** (`services/core/ai/tools/plan.ts`) — the auto-repair loop (bad-JSON → retry,
  `checkSection` trips → retry, throw after 2 fails). Composes real `extractJson` + `zSection` + `checkSection`.
- **`generate-artifact` through `runTool`** — assert the exact `TurnEvent` sequence (start → outline → per-beat
  status/patch → cover injects a background → finish) against the in-memory generation store.
- **`services/core/ai/chat.ts` `runChat`** — toolset assembly. The one place to prove "fake stream → correct
...
1. **Gated pure helpers** (quick wins — stop crashing on import under the client build; some need a one-line
   ...
   `ThemeEditor` `shadowCss`↔`inferShadow` round-trip. Also `app/stores/theme` `setAppTheme` /
   `adoptUserPrefs`, where the account row is the source of truth and `localStorage` is the first-frame
   cache: the invariants to assert are that a set writes both and posts once, that an unchanged id posts
   nothing, and that adopting the server's value never echoes it back. The module top-level-imports
   `@solidjs/router` (for `overlayThemeVars`), so it cannot be imported under the `unit` project today;
   the wire contract underneath it is covered by `model/__tests__/workspace.test.ts` and
   `services/api/__tests__/account.itest.ts`.
